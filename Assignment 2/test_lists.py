...
def compare_lists(num, measurements=3, verbose=None):
    """
    :param num: Number of insertions at the beginning of a storage_list
    ...
    # set up storage_list to store averages
    differences_pylist = []
    differences_mylist = []
    ...
    for _ in range(measurements):
        ...
        for iteration in range(num):
            python_list.insert(0, iteration)  # insert at start of storage_list; could be any value instead of iteration
        ...
        for iteration in range(num):
            my_singly_linked_list.prepend(
                iteration)  # insert at start of storage_list; could be any value instead of iteration
    ...
    if verbose:
        print(f"The built-in Python storage_list takes on average {sum(differences_pylist) / 3}ns.")
        print(f"My implementation of a singly linked storage_list takes on average {sum(differences_mylist) / 3}ns.")
        if difference > 0:
            print(
                f"With {num} insertions at the beginning, my implementation of a singly linked storage_list"
                f" is {difference} ns or"
                f" {difference / 1000} us or"
                f" {difference / 1000000000} s faster than a built-in Python storage_list."
            )
        elif difference < 0:
            print(
                f"With {num} insertions at the beginning, the built-in Python storage_list"
                f" is {-difference}ns or"
                f" {-difference / 1000}us or"
                f" {-difference / 1000000000}s faster than my implementation of a singly linked storage_list."
            )
...
def test_compare_lists():
    ...
    for num_element in num_elements:
        ...
        if difference > 0:  # my storage_list is faster
            print(f"{num_element}: My_SingleLinkedList is {difference / 1000:.2f}us faster")
        elif difference < 0:  # built-in Python storage_list is faster
            print(f"{num_element}:Built-in Python storage_list is {-difference / 1000:.2f}us faster")
