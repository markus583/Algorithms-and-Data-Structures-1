...
class MyLinkedList:
    ...
    def remove_first(self) -> int:
        ...
        if self._header.get_next_node() is not None:  # make sure len(list) > 1
            self._header = self._header.get_next_node()  # move header 1 to the right
        else:  # len(list) == 1 --> just remove header
            self._header = None
            self._tail = None
        return first_node
    ...
    def contains(self, integer_val: int) -> bool:
        ...
        if not isinstance(integer_val, int):
            raise ValueError(f"{integer_val} is not an integer!")
        current = self._header
        while current is not None:  # go through all nodes in list
    ...
    def to_list(self):
        ...
        """
        list_list = []  # set up empty list
        current = self._header  # start with header
        while current is not None:  # go through all nodes in list
            list_list.append(current.get_data())  # add value to list
            current = current.get_next_node()  # go to next item in list
        return list_list
